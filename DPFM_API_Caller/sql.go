...
import (
	...
	dpfm_api_processing_formatter "data-platform-api-purchase-requisition-creates-rmq-kube/DPFM_API_Processing_Formatter"
...
func (c *DPFMAPICaller) partnerCreateSql(
	...
	for _, partnerData := range *subfuncSDC.Message.Partner {
		res, err := c.rmq.SessionKeepRequest(ctx, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": partnerData, "function": "PurchaseRequisitionPartner", "runtime_session_id": sessionID})
		if err != nil {
			err = xerrors.Errorf("rmq error: %w", err)
			*errs = append(*errs, err)
			return nil
...
func (c *DPFMAPICaller) addressCreateSql(
	...
	for _, addressData := range *subfuncSDC.Message.Address {
		res, err := c.rmq.SessionKeepRequest(ctx, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": addressData, "function": "PurchaseRequisitionAddress", "runtime_session_id": sessionID})
		if err != nil {
			err = xerrors.Errorf("rmq error: %w", err)
			*errs = append(*errs, err)
			return nil
...
func (c *DPFMAPICaller) headerUpdateSql(
	...
	if headerIsUpdate(headerData) {
		res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": headerData, "function": "PurchaseRequisitionHeader", "runtime_session_id": sessionID})
		if err != nil {
			err = xerrors.Errorf("rmq error: %w", err)
			*errs = append(*errs, err)
			return nil
...
func (c *DPFMAPICaller) itemUpdateSql(
	...
	for _, item := range header.Item {
		...
		if itemIsUpdate(&itemData) {
			res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": itemData, "function": "PurchaseRequisitionItem", "runtime_session_id": sessionID})
			if err != nil {
				err = xerrors.Errorf("rmq error: %w", err)
				*errs = append(*errs, err)
				return nil
...
func (c *DPFMAPICaller) partnerUpdateSql(
	...
	for _, partner := range header.Partner {
		...
		if partnerIsUpdate(&partnerData) {
			res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": partnerData, "function": "PurchaseRequisitionPartner", "runtime_session_id": sessionID})
			if err != nil {
				err = xerrors.Errorf("rmq error: %w", err)
				*errs = append(*errs, err)
				return nil
...
func (c *DPFMAPICaller) addressUpdateSql(
	...
	for _, address := range header.Address {
		...
		if addressIsUpdate(&addressData) {
			res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": addressData, "function": "PurchaseRequisitionAddress", "runtime_session_id": sessionID})
			if err != nil {
				err = xerrors.Errorf("rmq error: %w", err)
				*errs = append(*errs, err)
				return nil
...
func headerIsUpdate(header *dpfm_api_processing_formatter.HeaderUpdates) bool {
	purchaseRequisition := header.PurchaseRequisition

	return !(purchaseRequisition == 0)
}
...
func itemIsUpdate(item *dpfm_api_processing_formatter.ItemUpdates) bool {
	...
	return !(purchaseRequisition == 0 || purchaseRequisitionItem == 0)
}

func partnerIsUpdate(partner *dpfm_api_processing_formatter.PartnerUpdates) bool {
	purchaseRequisition := partner.PurchaseRequisition
	partnerFunction := partner.PartnerFunction
	businessPartner := partner.BusinessPartner

	return !(purchaseRequisition == 0 || partnerFunction == "" || businessPartner == 0)
}

func addressIsUpdate(address *dpfm_api_processing_formatter.AddressUpdates) bool {
	purchaseRequisition := address.PurchaseRequisition
	addressID := address.AddressID

	return !(purchaseRequisition == 0 || addressID == 0)
}
